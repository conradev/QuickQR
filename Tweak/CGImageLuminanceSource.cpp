...
CGImageRef CGImageLuminanceSource::createImageFromBuffer(CVPixelBufferRef buffer, int left, int top, int width, int height) {
    ...
    unsigned char *baseAddress = (unsigned char *)CVPixelBufferGetBaseAddress(buffer); 
    ...
    if (newBytesPerRow == bytesPerRow) {
        ...
        for(int y=0; y<height; y++) {
            memcpy(bytes+y*newBytesPerRow, baseAddress+left*4+(top+y)*bytesPerRow, newBytesPerRow);
        }
    }
    ...
    CGContextRef newContext = CGBitmapContextCreate(bytes, width, height, 8, newBytesPerRow, colorSpace, kCGBitmapByteOrder32Little | kCGImageAlphaNoneSkipFirst);
...
CGImageLuminanceSource::CGImageLuminanceSource(CVPixelBufferRef buffer, int left, int top, int width, int height) {
...
CGImageLuminanceSource::CGImageLuminanceSource(CGImageRef cgimage, int left, int top, int width, int height)  {
    init(cgimage, left, top, width, height);
}

void CGImageLuminanceSource::init(CGImageRef cgimage) {
    init(cgimage, 0, 0, (int)CGImageGetWidth(cgimage), (int)CGImageGetHeight(cgimage));
}
